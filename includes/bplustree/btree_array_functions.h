...
// template <class T>
// int first_ge(const T data[], int n, const T& entry);    // return the index
// of the first element in data that is not less than entry

// template <>
// int first_ge(const std::string data[], int n, const std::string& entry); //
...
template <class T>
void split(
    T data1[], int &n1, T data2[],
    int &n2); // move n/2 elements from the right of data1 and move to data2
...
// ANCHOR - first_ge
template <class T>
inline int first_ge(const T data[], int n,
                    const T &entry) { // return the index of the first element
...
template <class T> inline int first_gt(const T data[], int n, const T &entry) {
  int i = 0;
  while (i < n && data[i] <= entry) {
    i++;
  }
...
int index_of_maximal(T data[],
  ...
  for (int i = 1; i < n; ++i) {
    if (data[i] >
        data[max_index]) { // if data[i] is greater than data[max_index]
...
void ordered_insert(
  ...
  int index = first_ge(data, n, entry); // find the index of the first element
                                        // in data that is not less than entry
...
template <class T> void remove_entry(T data[], int &n, int index) {
