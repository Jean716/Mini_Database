...
template <typename K, typename V> struct MPair {
  ...
  MPair(const K &k, const V &v) : key(k) {
    ...
    // cout << "MPair(key, value) constructor called with key: " << key << ",
    // value: " << v << endl;
  }
  MPair(const K &k, const vector<V> &vlist) : key(k), value_list(vlist) {
    // cout << "MPair(key, vector) constructor called with key: " << key <<
    // endl;
  }
  ...
  friend bool operator==(const MPair<_K, _V> &lhs, const MPair<_K, _V> &rhs);
  ...
  friend MPair<K, V> operator+(const MPair<K, V> &lhs, const MPair<K, V> &rhs) {
    ...
    result.value_list.insert(result.value_list.end(), rhs.value_list.begin(),
                             rhs.value_list.end());
    return result;
  }
};
...
inline bool operator==(const MPair<K, V> &lhs, const MPair<K, V> &rhs) {
...
template <typename V>
inline bool operator<(const MPair<std::string, V> &lhs,
               const MPair<std::string, V> &rhs) {
  if (is_number(lhs.key) && is_number(rhs.key)) {
    return stod(lhs.key) < stod(rhs.key);
  }
  return lhs.key < rhs.key;
}
...
public:
  ...
  void insert(const K &k, const V &v) {
    // cout << "MMap::insert called with key: " << k << ", value: " << v <<
    // endl;
  ...
  //  Operations:
  bool contains(const K &key) const { return mmap.contains(MPair<K, V>(key)); }
  vector<V> &get(const K &key) { return mmap.get(MPair<K, V>(key)).value_list; }

  Iterator find(const K &key) { return Iterator(mmap.find(MPair<K, V>(key))); }
  ...
  vector<V> range_query(const K &key, const string &op) {
    ...
    if (op == "<") {
      ...
      for (auto it = begin(); it != end; ++it) {
        result.insert(result.end(), it->value_list.begin(),
                      it->value_list.end());
      }
    } else if (op == "<=") {
      auto end = upper_bound(key);
